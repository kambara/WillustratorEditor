...
class util.AdvancedDrawingUIObject extends UIObject {
	//
	// ただの線
	//
	...
	function drawStyleLine(x1:Number, y1:Number, x2:Number, y2:Number, style:Object) {
		var lineWidth = (style.lineWidth != undefined) ? style.lineWidth : 1;
		var lineColor = (style.lineColor != undefined) ? style.lineColor : 0x000000;
		var lineAlpha = (style.lineAlpha != undefined) ? style.lineAlpha : 100;
	...
	// ただの長方形
	//
	function drawFillRect(x, y, w, h, style:Object) {
		var fillColor = (style.fillColor != undefined) ? style.fillColor : 0xFFFFFF;
		var fillAlpha = (style.fillAlpha != undefined) ? style.fillAlpha : 100;
		beginFill(fillColor, fillAlpha);
		drawRectLine(x, y, w, h, style);
		endFill();
	}
	function drawRectLine(x, y, w, h, style:Object) {
		var lineWidth = (style.lineWidth != undefined) ? style.lineWidth : 1;
		var lineColor = (style.lineColor != undefined) ? style.lineColor : 0x000000;
		var lineAlpha = (style.lineAlpha != undefined) ? style.lineAlpha : 100;
		lineStyle(lineWidth, lineColor, lineAlpha);
